...
func update(p_curve: Curve3D) -> void:
	...
	var _baked_up_vectors := curve.get_baked_up_vectors()
	var _baked_tilts := curve.get_baked_tilts()
	...
	vertices += get_in_place_transformed(
		get_cap3d(false),
		Transform3D(Basis(), baked_points[0])
	)
	...
	vertices += get_in_place_transformed(
		get_cap3d(true),
		Transform3D(Basis(), baked_points[len(baked_points)-1])
	)
	...
	#for vertex in vertices:
		#if i_debug == 0:
			#Cavedig.needle(self,Transform3D(Basis(), vertex),Vector3(0.0, 1.0, 0.0), 0.013, 0.0012)
		#elif i_debug == 1:
			#Cavedig.needle(self,Transform3D(Basis(), vertex),Vector3(0.4, 0.6, 0.0), 0.016, 0.0010)
		#elif i_debug == 2:
			#Cavedig.needle(self,Transform3D(Basis(), vertex),Vector3(0.0, 0.2, 0.8), 0.019, 0.0008)
		#elif i_debug == 3:
			#Cavedig.needle(self,Transform3D(Basis(), vertex),Vector3(0.8, 0.2, 0.4), 0.022, 0.0006)
			#
		#if i_debug == 3:
			#i_debug = 0
		#else:
...
# Somehow the curve isn't rendered right. Let's break it down and see what
# works.
func _static_cube_extrusion_experiment() -> MeshInstance3D:
...
func _ready() -> void:
	...
	curve.add_point(Vector3(1.5, 2.0, 1.0))
	curve.add_point(Vector3(2.6, 2.3, 2.0))
	curve.add_point(Vector3(1.9, 3.7, 3.0))
	curve.add_point(Vector3(4.1, 2.4, 4.0))
